/*
	...
	I have never used the MMU before, and starting with
	the Pi is an awful idea! It's way more complicated than
	x86 for example, so it's terrible for a first-time MMU
	implementation. So.... I coppied it straight off:
...
void mmu_init()
{
    ...
    unsigned long r, b, *paging=(unsigned long*)&__end;
    ...
    if(r&(0xF<<28)/*4k*/ || b<1/*36 bits*/) {
        uart_print("ERROR: 4k granule or 36 bit address space not supported\n");
        return;
    }
    ...
    asm volatile ("msr ttbr0_el1, %0" : : "r" ((unsigned long)&__end + TTBR_CNP));
    // upper half, kernel space
    asm volatile ("msr ttbr1_el1, %0" : : "r" ((unsigned long)&__end + TTBR_CNP + PAGESIZE));
